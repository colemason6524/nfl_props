...
- 2026 pbp 404s until the season's first games are published; ratings state
  ...
  rebuild.
- Team-total parser verified against real Week-1 payloads (2026-09-08/09):
  matched every team-total market present (`team_totals_found` = 32 = 16
  games × 2, `matched_no_outcomes: []`); `unmatched_total_desc` is player
  props and player/team TD markets, correctly ignored.
...
- Tiers: Core needs EV in [2%, 8%], edge ≥ 2%, both teams ≥ 3 current-season
  games; otherwise Lean at best; oversized EV always Watch. Only the max-EV
  side of each (game, market) family can be Core/Lean (`OPPOSITE_SIDE` →
  Watch).
...
1. Daily board via the VM systemd timer; check Bovada coverage diagnostics.
2. Week 1 (Sept 2026): grading live from the first week; no retuning.
