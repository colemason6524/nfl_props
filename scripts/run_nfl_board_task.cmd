...
REM Defaults: ProjectDir = repo root, PythonExe = .venv, NFL_SEND_DISCORD = false.
...
if not defined NFL_SEND_DISCORD (
  for /f "tokens=2*" %%A in ('reg query "HKCU\Environment" /v NFL_SEND_DISCORD 2^>nul') do set "NFL_SEND_DISCORD=%%B"
)
...
if "%NFL_SEND_DISCORD%"=="" set "NFL_SEND_DISCORD=false"

set "ARGS=run_board.py"
if /I "%NFL_SEND_DISCORD%"=="true" set "ARGS=run_board.py --discord"
if /I "%NFL_SEND_DISCORD%"=="1" set "ARGS=run_board.py --discord"
if /I "%NFL_SEND_DISCORD%"=="yes" set "ARGS=run_board.py --discord"
